...
include( plugin_dir_path( __FILE__ ) . 'mt-events.php' );
...
function my_tickets_menu() {
	...
	if ( function_exists( 'add_object_page' ) ) {
		add_object_page( __( 'My Tickets', 'my-tickets' ), __( 'My Tickets', 'my-tickets' ), $permission, 'my-tickets', 'mt_settings', $icon_path . '/tickets.png' );
	}
	...
	add_submenu_page( 'my-tickets', __( 'My Tickets', 'my-tickets' ), __( 'Ticket Settings', 'my-tickets' ), $permission, 'mt-ticketing', 'mt_ticketing_settings' );
	add_submenu_page( 'my-tickets', __( 'My Tickets > Reports', 'my-tickets' ), __( 'Reports', 'my-tickets' ), $permission, 'mt-reports', 'mt_reports_page' );
...
function mt_public_enqueue_scripts() {
	...
	wp_localize_script( 'mt.public', 'mt_ajax',
		array(
			...
			<div class="postbox">
				...
				<div id="support" class="inside resources">
					...
					<ul>
						...
						<li><a href="http://wordpress.org/extend/plugins/access-monitor/"><?php _e('Rate this plug-in','my-tickets'); ?></a></li>
					</ul>
				</div>
			</div>
		</div>
...
function mt_money_format( $price ) {
	if ( is_numeric( $price ) ) {
		...
		return $symbol . '<span class="price">' . money_format( '%i', $price ) . '</span>';
	} else {
		return '';
	}
}
...
/**
 *  Basic fallback for Windows systems.
 */
