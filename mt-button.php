...
function mt_tickets_left( $pricing, $available ) {
	...
	foreach ( $pricing as $options ) {
		if ( $available != 'inherit' ) {
			$sold = $sold + $options['sold'];
		} else {
			$tickets = $options['tickets'] - $options['sold'];
			$total   = $total + $tickets;
		}
	}
	if ( $available != 'inherit' ) {
...
function mt_add_to_cart() {
	if ( ! isset( $_POST['mt_add_to_cart'] ) ) {
		...
		$saved    = mt_save_data( array( 'event_id' => $event_id, 'options' => $options ) );
	}
	if ( $saved ) {
 ...
 * @param $type
 */
function mt_register_message( $context, $type ) {
	global $mt_message;
	$mt_message = mt_get_message( $context, $type );
}

function mt_get_message( $context, $type ) {
